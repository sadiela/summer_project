...
plot.mediation <- function(med, 
  ...
  for(i in 1:21771){
    print(i)
    if(med$strayfromLOD[i] >= breaks[1] & med$strayfromLOD[i] < breaks[17]){
      med$col[i] <- "#2d7c00"
    }
    if(med$strayfromLOD[i] >= breaks[17] & med$strayfromLOD[i] <= breaks[18]){
      med$col[i] <- "#fc00ef"
    }
  }
  ...
  colnumbers <- which(med$col =="#fc00ef")
  for(i in colnumbers){
    text(x = med$GMB[i] + 20, y = med$LOD[i], labels = med$SYMBOL[i], col = "black", pos = 4, cex = 0.8)
  }
}
